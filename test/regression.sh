...
if RIPWIRE_BIN="$BIN" bash "$ROOT/test/redisqualitycachecheck.sh"; then
    ...
    no "Redis derived cache gate (test/redisqualitycachecheck.sh failed)"
fi
...
if RIPWIRE_BIN="$BIN" bash "$ROOT/test/redisclientcheck.sh" >/dev/null 2>&1; then
    ...
    no "Redis transport gate (test/redisclientcheck.sh failed)"
    RIPWIRE_BIN="$BIN" bash "$ROOT/test/redisclientcheck.sh" 2>&1 | grep -E 'FAIL|error:|fatal error:' | head -12
fi

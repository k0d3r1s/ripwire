...
inline constexpr char kHelpTail[] =
        ...
        "                               cache_backend verifies Redis PING, SET NX EX 5, GET, MGET, EXPIRE and DEL using\n"
        "                               one disposable random canary. Partial ACL capability is ok=0; cleanup failure\n"
        "                               leaves only its five-second TTL. transport=loopback_tcp|unix|remote_tcp, db=,\n"
        "                               scope= (16 hex fingerprint) and prefix= (rw:v1 plus full namespace/project SHA-256 hashes,\n"
        "                               no trailing colon, for cleanup) expose no endpoint or credentials. remote_tcp requires explicit plaintext opt-in; failures= lists\n"
        "                               bounded live and previously observed backend failure classes.\n"
        ...
        "                               Redis is a disposable shared cache, not a source of truth. If unreachable,\n"
        "                               ripwire recomputes from source without local persistence.\n"
